...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const songsRemainingDiv = document.getElementById('songs-remaining');
    let currentSongIndex = 0; 
    let audio = new Audio(); //An instance of the Audio object used to play the songs.
    let score = 0;
    let remainingGuesses = 3;
    let wrongLetters = []; //Stores an array of wrong guesses.
    let currentSong = null; //Stores the current song object.
    ...
    updateSongsRemaining();
//The updateSongsRemaining function is called to initialize the display of the remaining songs.


    function playNextSong() {
        //currentsongindex when the song play it value is 1 and increase so on and otherwise is 0. no song play
        if (currentSongIndex >= songs.length) {
            resultDiv.innerText = "You've reached the end of the song list.";
            letterInputsDiv.innerHTML = ''; // Clear the letter inputs
            return;
    ...
    function createLetterInputs(length) {
        ...
        for (let i = 0; i < length; i++) {
            const input = document.createElement('input');
            input.type = 'text';
            input.maxLength = 1; //restricts the input to a single character.
            input.dataset.index = i; 
            input.addEventListener('input', onLetterInput);
            input.addEventListener('keydown', onLetterKeyDown); // Add keydown event listener
            letterInputsDiv.appendChild(input);
        ...
        if (letterInputsDiv.firstChild) {
            letterInputsDiv.firstChild.focus();
            //sets the focus on the first input field. This allows the user to start typing immediately without having to click on the input field.
        }
    }
  ...
  //if statement checks two conditions:
   ...
   // nextInput: This ensures that there is a next input field to move to.
    //input.value: This checks if the current input field has a value (i.e., the user has typed something into it).
    function onLetterInput(event) {
        const input = event.target;
        const nextInput = input.nextElementSibling;
        if (nextInput && input.value) {
            nextInput.focus();
            //nextInput.focus() sets the focus to the next input field, allowing the user to continue typing in the next field without manually clicking on it.
        }
    }

    function onLetterKeyDown(event) {
        const input = event.target;
        //the function captures the input field that the user is interacting with. This allows the function to perform operations on that specific input field.
        const prevInput = input.previousElementSibling;
    ...
    function skipSong() {
        resultDiv.innerText = `Skipped. The correct answer was ${currentSong.title}`;
        // currentSongIndex++;
        createNextSongButton();
    }
